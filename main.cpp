...
int main(int argc, char **argv)
{
    ...
    std::vector<double> times = wq::run_queries(opt, [&](int t, double ms, const AttemptResult* posix, const wq::RawDnsResult* rawdns)
    {
        if (rawdns)
        {
            ...
            if (rd.rc != 0)
            {
                if (opt.ndjson)
                {
                    ...
                    switch (rd.kind)
                    {
                        case wq::RawDnsErrorKind::NotAvailable:
                            out = wq::build_ndjson_ldns_not_available(t, ms, rd.error, opt);
                            break;
                        case wq::RawDnsErrorKind::InitFailed:
                            out = wq::build_ndjson_rawdns_init_failed(t, ms, rd.error, opt);
                            break;
                        case wq::RawDnsErrorKind::InvalidQname:
                        case wq::RawDnsErrorKind::QueryFailed:
                        default:
                            out = wq::build_ndjson_rawdns_error_with_type(t, ms, rd.error, opt.qtype);
                            break;
                    }
                ...
                {
                    std::scoped_lock lk(g_print_mtx);
                    std::println("try {}: {:.3f} ms - raw DNS error: {}", t, ms, rd.error);
                }
                return;
            }
            ...
            if (opt.ndjson)
            {
                std::string out = wq::build_ndjson_rawdns_success(
                    t, ms, opt.qtype, rd.rcode,
                    rd.f_aa, rd.f_tc, rd.f_rd, rd.f_ra, rd.f_ad, rd.f_cd,
                    rd.answer_count, rd.authority_count, rd.additional_count, rd.answers);
                std::scoped_lock lk(g_print_mtx);
                std::print("{}\n", out);
            }
            else if (opt.json)
            ...
            else
            {
                ...
                std::println(
                    ...
                    t, ms, rd.rcode, rd.f_aa, rd.f_tc, rd.f_rd, rd.f_ra, rd.f_ad, rd.f_cd, rd.answer_count);
            }
            return;
        }
        ...
        if (res.rc != 0)
        {
            if (opt.ndjson)
            {
                std::string out = wq::build_ndjson_getaddrinfo_error(t, ms, res.rc);
                std::scoped_lock lk(g_print_mtx);
                std::print("{}\n", out);
            }
            else if (opt.json)
            ...
            {
                std::scoped_lock lk(g_print_mtx);
                std::println("try {}: {:.3f} ms - error: {}", t, ms, res.error);
            }
            return;
        }
        ...
        if (opt.ndjson)
        {
            std::string out = wq::build_ndjson_normal(t, ms, res.canon, res.entries, res.ptrs);
            std::scoped_lock lk(g_print_mtx);
            std::print("{}\n", out);
        }
        else if (opt.json)
        ...
        else
        {
            ...
            std::print("{}", wq::format_try_footer_text(t, ms, res.entries.size(), res.canon));
        }
    });
    ...
    if (!times.empty())
    {
        ...
        if (opt.json && !opt.ndjson)
        {
            std::string out = wq::build_final_json(opt, ag.min, ag.avg, ag.max, ag.percentiles, attempts);
            std::print("{}\n", out);
        }
        else if (!opt.ndjson)
        {
            std::print("{}", wq::format_summary_text(ag.min, ag.avg, ag.max, times.size()));
