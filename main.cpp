...
struct Options {
    ...
    std::string qtype;      // when non-empty, enable raw DNS path (e.g., "A","AAAA","TXT",...)
...
static void print_usage(const char *prog) {
    ...
    std::println("  --ndjson           Output each attempt as a single JSON line (NDJSON)");
    std::println("  --pctl LIST        Comma-separated percentiles for summary (e.g., 50,90,99)");
    std::println("  --dedup            Fold duplicate results per attempt");
    std::println("  --type RR          Raw DNS mode (ldns): A,AAAA,CNAME,NS,MX,TXT,SOA,CAA,SRV,DS,DNSKEY,PTR");
    ...
    std::println("  --timeout MS       Query timeout in milliseconds (default: 2000)");
    std::println("  --tcp              Force TCP transport (default: UDP with TCP fallback)");
    std::println("  -h, --help         Show this help");
    std::println("");
    std::println("Examples:");
    std::println("  {} example.com", prog);
    std::println(
        "  {} --tries 5 --family inet6 --v4mapped --all www.google.com", prog);
}

static const char *family_str(int af);

static void reverse_lookup_list(addrinfo *res, bool namereqd) {
    ...
    for (addrinfo *ai = res; ai != nullptr; ai = ai->ai_next) {
        ...
        int rc = getnameinfo(ai->ai_addr, ai->ai_addrlen,
                             name, sizeof(name),
                             nullptr, 0,
                             flags);
        if (rc == 0) {
            std::println("  PTR: [{}] {} -> {}", family_str(ai->ai_family),
                         ip.empty() ? "-" : ip.c_str(), std::string_view{name});
        } else {
            std::println("  PTR: [{}] {} -> <{}>", family_str(ai->ai_family),
                         ip.empty() ? "-" : ip.c_str(), gai_strerror(rc));
        }
    }
}
...
static std::vector<PtrItem> do_reverse_for_entries(
    const std::vector<Entry> &entries, bool namereqd) {
    ...
    for (const auto &e: entries) {
        ...
        if (e.af == AF_INET) {
            ...
            int rc = getnameinfo(reinterpret_cast<sockaddr *>(&sin),
                                 sizeof(sin), name, sizeof(name), nullptr, 0,
                                 flags);
            item.rc = rc;
            ...
            int flags = NI_NOFQDN | (namereqd ? NI_NAMEREQD : 0);
            int rc = getnameinfo(reinterpret_cast<sockaddr *>(&sin6),
                                 sizeof(sin6), name, sizeof(name), nullptr, 0,
                                 flags);
            item.rc = rc;
            if (rc == 0) item.name = std::string{name};
            else item.error = gai_strerror(rc);
...
static void print_entries(const std::vector<Entry> &entries) {
    for (const auto &e: entries) {
        if (e.port)
            std::println("  - [{}] {}  socktype={}  proto={}  port={}",
                         family_str(e.af), e.ip, socktype_str(e.socktype),
                         proto_str(e.protocol), e.port);
        else
            std::println("  - [{}] {}  socktype={}  proto={}", family_str(e.af),
                         e.ip, socktype_str(e.socktype), proto_str(e.protocol));
    }
}
...
static void print_ptrs(const std::vector<PtrItem> &ptrs) {
    for (const auto &p: ptrs) {
        if (p.rc == 0)
            std::println("  PTR: [{}] {} -> {}", family_str(p.af), p.ip,
                         p.name);
        else
            std::println("  PTR: [{}] {} -> <{}>", family_str(p.af), p.ip,
                         p.error);
...
static size_t print_addrinfo_list(addrinfo *res) {
    size_t count = 0;
    char buf[INET6_ADDRSTRLEN]{};
    for (addrinfo *ai = res; ai != nullptr; ai = ai->ai_next) {
        std::string addr;
        ...
        if (!addr.empty()) {
            ...
            if (port)
                std::println("  - [{}] {}  socktype={}  proto={}  port={}",
                             family_str(ai->ai_family), addr,
                             socktype_str(ai->ai_socktype),
                             proto_str(ai->ai_protocol), port);
            else
                std::println("  - [{}] {}  socktype={}  proto={}",
                             family_str(ai->ai_family), addr,
                             socktype_str(ai->ai_socktype),
                             proto_str(ai->ai_protocol));
        }
    }
    return count;
}
...
static bool parse_args(int argc, char **argv, Options &opt) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "-h"sv || a == "--help"sv) {
            ...
            else if (a.size() > 11 && a.substr(10, 1) == "="sv)
            ...
            try { opt.concurrency = std::stoi(val); } catch (...) {
            ...
            for (char ch : val) {
                if (ch == ',') {
                    ...
                    std::println("invalid --pctl character: {}", std::string(1, ch));
                    return false;
                }
            }
            if (!num.empty()) {
            ...
            try { opt.tries = std::stoi(val); } catch (...) {
            ...
            if (a == "--rd"sv && i + 1 < argc) val = argv[++i];
                ...
                val = std::string(a.substr(5));
            else { std::println("invalid --rd usage"); return false; }
            if (val == "on" || val == "1" || val == "true") opt.rd = true;
            else if (val == "off" || val == "0" || val == "false") opt.rd = false;
            else { std::println("invalid --rd value: {}", val); return false; }
        } else if (a.rfind("--do", 0) == 0) {
            std::string val;
            if (a == "--do"sv && i + 1 < argc) val = argv[++i];
            else if (a.size() > 5 && a.substr(4, 1) == "="sv)
                val = std::string(a.substr(5));
            else { std::println("invalid --do usage"); return false; }
            if (val == "on" || val == "1" || val == "true") opt.do_bit = true;
            else if (val == "off" || val == "0" || val == "false") opt.do_bit = false;
            else { std::println("invalid --do value: {}", val); return false; }
        } else if (a.rfind("--timeout", 0) == 0) {
            std::string val;
            if (a == "--timeout"sv && i + 1 < argc) val = argv[++i];
            else if (a.size() > 10 && a.substr(9, 1) == "="sv)
                val = std::string(a.substr(10));
            else { std::println("invalid --timeout usage"); return false; }
            try { opt.timeout_ms = std::stoi(val); } catch (...) {
                std::println("invalid --timeout value: {}", val); return false; }
...
int main(int argc, char **argv) {
    ...
    if (!opt.json && !opt.ndjson) {
        ...
        std::println(
            ...
            opt.addrconfig ? "on" : "off", opt.canonname ? "on" : "off",
            opt.all ? "on" : "off", opt.v4mapped ? "on" : "off",
            opt.numeric_host ? "on" : "off");
        std::println("Socktype: {}  Protocol: {}  Service: {}",
                     socktype_str(opt.socktype), proto_str(opt.protocol),
                     opt.service.empty() ? "(none)" : opt.service.c_str());
        std::println(
            "Reverse: {}  NI_NAMEREQD: {}  Concurrency: {}  JSON: {}  Dedup: {}",
            opt.reverse ? "on" : "off", opt.ni_namereqd ? "on" : "off",
            opt.concurrency, opt.json ? "on" : "off", opt.dedup ? "on" : "off");
        if (!opt.qtype.empty()) {
            std::println("Raw DNS: type={} ns={} rd={} do={} timeout_ms={} tcp={}",
                         opt.qtype,
    ...
    auto attempt_fn = [&](int t) {
        ...
        if (!opt.qtype.empty()) {
            ...
            if (st != LDNS_STATUS_OK || !res) {
                ...
                ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1e - t0).count();
                times[t - 1] = ms;
                ...
                if (opt.ndjson) {
                    ...
                    os << R"(,"raw_dns":{"type":")" << json_escape(opt.qtype) << R"(","ns":")" << json_escape(opt.ns)
                       << R"(","rd":)" << (opt.rd?"true":"false") << R"(,"do":)" << (opt.do_bit?"true":"false")
                       << R"(,"timeout_ms":)" << opt.timeout_ms << R"(,"tcp":)" << (opt.tcp?"true":"false") << "}";
                    os << "}";
                    std::scoped_lock lk(g_print_mtx);
                    std::print("{}\n", os.str());
                } else if (opt.json) {
                    AttemptResult ar{}; ar.ms = ms; ar.rc = -1; ar.error = std::move(err); attempts[t-1] = std::move(ar);
                } else {
                    std::scoped_lock lk(g_print_mtx);
                    std::println("try {}: {} ms - raw DNS error: {}", t, ms, err);
                }
            ...
            if (opt.timeout_ms >= 0) {
                struct timeval tv{.tv_sec = opt.timeout_ms/1000, .tv_usec = (opt.timeout_ms%1000)*1000};
            ...
            if (!name) {
                ...
                if (opt.ndjson) {
                    ...
                    os << "\"try\":" << t << ",\"ms\":" << ms << ",\"rc\":-1";
                    os << R"(,"error":")" << json_escape(err) << R"(")";
                    os << R"(,"raw_dns":{"type":")" << json_escape(opt.qtype) << R"("}})";
                    std::scoped_lock lk(g_print_mtx); std::print("{}\n", os.str());
                } else if (opt.json) {
                    AttemptResult ar{}; ar.ms = ms; ar.rc = -1; ar.error = std::move(err); attempts[t-1] = std::move(ar);
                } else {
                    std::scoped_lock lk(g_print_mtx);
                    std::println("try {}: {} ms - raw DNS error: invalid qname", t, ms);
                }
            ...
            if (qtype == 0) { // fallback for unknown string
                // minimal mapping for common types
                if (opt.qtype == "A") qtype = LDNS_RR_TYPE_A;
                else if (opt.qtype == "AAAA") qtype = LDNS_RR_TYPE_AAAA;
                else if (opt.qtype == "CNAME") qtype = LDNS_RR_TYPE_CNAME;
                else if (opt.qtype == "NS") qtype = LDNS_RR_TYPE_NS;
                else if (opt.qtype == "MX") qtype = LDNS_RR_TYPE_MX;
                else if (opt.qtype == "TXT") qtype = LDNS_RR_TYPE_TXT;
                else if (opt.qtype == "SOA") qtype = LDNS_RR_TYPE_SOA;
                else if (opt.qtype == "CAA") qtype = LDNS_RR_TYPE_CAA;
                else if (opt.qtype == "SRV") qtype = LDNS_RR_TYPE_SRV;
                else if (opt.qtype == "DS") qtype = LDNS_RR_TYPE_DS;
                else if (opt.qtype == "DNSKEY") qtype = LDNS_RR_TYPE_DNSKEY;
                else if (opt.qtype == "PTR") qtype = LDNS_RR_TYPE_PTR;
                else qtype = LDNS_RR_TYPE_A;
            }
            ...
            if (st != LDNS_STATUS_OK || !pkt) {
                ...
                if (opt.ndjson) {
                    ...
                    std::scoped_lock lk(g_print_mtx);
                    std::println("try {}: {} ms - raw DNS error: ldns query failed", t, ms);
                }
            ...
            if (opt.ndjson) {
                std::ostringstream os; os << "{";
                os << "\"try\":" << t << ",\"ms\":" << ms << ",\"rc\":0";
                os << R"(,"raw_dns":{"type":")" << json_escape(opt.qtype) << R"(","rcode":)" << rcode
                   ...
                   << R"(,"counts":{"answer":)" << an << R"(,"authority":)" << au << R"(,"additional":)" << ad << "}";
                ...
                for (size_t i = 0; i < an; ++i) {
                    ...
                    if (s) { os << R"(")" << json_escape(std::string{s}).c_str() << R"(")"; LDNS_FREE(s); }
                    else { os << R"(")" << "" << R"(")"; }
                }
                os << "]}"; // close raw_dns and object
                std::scoped_lock lk(g_print_mtx); std::print("{}\n", os.str());
            } else if (opt.json) {
                AttemptResult ar{}; ar.ms = ms; ar.rc = 0; ar.error.clear(); attempts[t-1] = std::move(ar);
            } else {
                std::scoped_lock lk(g_print_mtx);
                std::println("try {}: {} ms - raw DNS rcode={} aa={} tc={} rd={} ra={} ad={} cd={} an={}",
                             t, ms, rcode, f_aa, f_tc, f_rd, f_ra, f_ad, f_cd, an);
            }
            ...
            ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
            times[t - 1] = ms;
            std::string err = "ldns not available: rebuild with ldns (pkg-config ldns) to enable raw DNS";
            if (opt.ndjson) {
                std::ostringstream os; os << "{";
                os << "\"try\":" << t << ",\"ms\":" << ms << ",\"rc\":-1";
                os << R"(,"error":")" << json_escape(err) << R"(")";
        ...
        if (opt.reverse)
            ptrs = do_reverse_for_entries(entries, opt.ni_namereqd);
        ...
        if (opt.ndjson) {
            ...
            os << "{";
            os << "\"try\":" << t << ",\"ms\":" << ms << ",\"rc\":0";
            if (!canon.empty()) os << R"(,"canon":")" << json_escape(canon) << "\"";
            os << ",\"addresses\":[";
            ...
            print_ptrs(ptrs);
            std::println("try {}: {} ms - {} address(es)", t, ms,
                         entries.size());
    ...
    if (!times.empty()) {
        ...
        long long minv = *min_it, maxv = *max_it;
        double avg = std::accumulate(times.begin(), times.end(), 0.0) /
                     static_cast<double>(times.size());
        // Precompute percentiles if requested
        std::vector<long long> sorted = times;
        std::sort(sorted.begin(), sorted.end());
        auto pct_value = [&](int p) -> long long {
            if (sorted.empty()) return 0;
            double pn = static_cast<double>(p);
            size_t n = sorted.size();
            size_t rank = static_cast<size_t>(std::ceil((pn / 100.0) * n));
            if (rank < 1) rank = 1;
            if (rank > n) rank = n;
        ...
        if (opt.json && !opt.ndjson) {
            ...
            std::ostringstream os;
            os << "{";
            os << "\"host\":\"" << json_escape(opt.host) << "\",";
            os << "\"family\":\"" << (opt.family == Family::Any
                                              ? "any"
                                              : opt.family == Family::IPv4
                                                    ? "inet"
                                                    : "inet6") << "\",";
            os << "\"tries\":" << opt.tries << ",";
            os << "\"service\":\"" << json_escape(opt.service) << "\",";
            os << "\"socktype\":\"" << json_escape(socktype_str(opt.socktype))
                    << "\",";
            os << "\"protocol\":\"" << json_escape(proto_str(opt.protocol)) <<
                    "\",";
            os << "\"flags\":{"
            ...
            for (int i = 0; i < opt.tries; ++i) {
                ...
                os << "\"try\":" << (i + 1) << ",\"ms\":" << ar.ms << ",\"rc\":"
                        << ar.rc;
                if (!ar.error.empty())
                    os << R"(,"error":")" << json_escape(ar.error) << "\"";
                if (!ar.canon.empty())
                    os << R"(,"canon":")" << json_escape(ar.canon) << "\"";
                os << ",\"addresses\":[";
            ...
            std::println(
                "summary: min={} ms, avg={:.2f} ms, max={} ms ({} tries)", minv,
                avg, maxv, times.size());
            if (!opt.pctl.empty()) {
                std::ostringstream os;
                os << "percentiles:";
