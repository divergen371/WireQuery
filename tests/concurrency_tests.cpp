...
static void test_batched_call_count_seq_par()
{
    {
        std::atomic<int> c{0};
        for_each_index_batched(17, 1, [&](int){ c.fetch_add(1, std::memory_order_relaxed); });
        assert_true(c.load() == 17, "seq: call count == total");
    }
    {
        std::atomic<int> c{0};
        for_each_index_batched(101, 4, [&](int){ c.fetch_add(1, std::memory_order_relaxed); });
...
static void test_batched_cancelable_exception_propagation()
{
    ...
    bool caught = false;
    try {
        for_each_index_batched_cancelable(50, 5, [&](int idx, const std::atomic<bool>&){
    ...
    assert_true(c.load() >= 0 && c.load() <= 50, "some work may have completed before cancel");
}
...
static void test_batched_cancelable_external_cancel()
{
    ...
    for_each_index_batched_cancelable(100, 8, [&](int, const std::atomic<bool>& flag){
...
static void test_batched_cancelable_seq_cancel_midway()
{
    std::atomic<int> c{0};
    Cancellation cancel;
    for_each_index_batched_cancelable(20, 1, [&](int idx, const std::atomic<bool>&){
        c.fetch_add(1, std::memory_order_relaxed);
...
{
    std::atomic<int> c{0};
    for_each_index_batched(1000, 8, [&](int){ c.fetch_add(1, std::memory_order_relaxed); });
...
static void test_pool_cancel_cooperative()
{
    ...
    for (int i = 0; i < 200; ++i)
    {
        pool.submit_cancelable([&](const std::atomic<bool>& flag){
            if (flag.load(std::memory_order_relaxed)) return; // cooperatively stop
            c.fetch_add(1, std::memory_order_relaxed);
        });
    }
    pool.cancel();
...
{
    std::atomic<int> c{0};
    for_each_index_batched(0, 4, [&](int){ c.fetch_add(1, std::memory_order_relaxed); });
...
static void test_batched_concurrency_le1_branches()
{
    ...
    {
        std::atomic<int> c{0};
        for_each_index_batched(10, 0, [&](int){ c.fetch_add(1, std::memory_order_relaxed); });
        assert_true(c.load() == 10, "concurrency=0 -> sequential");
    }
    {
        std::atomic<int> c{0};
        for_each_index_batched(10, -1, [&](int){ c.fetch_add(1, std::memory_order_relaxed); });
        assert_true(c.load() == 10, "concurrency=-1 -> sequential");
    }
    // cancelable variant
    {
        std::atomic<int> c{0};
        for_each_index_batched_cancelable(10, 0, [&](int, const std::atomic<bool>&){ c.fetch_add(1, std::memory_order_relaxed); });
        assert_true(c.load() == 10, "cancelable concurrency=0 -> sequential");
    }
    {
        std::atomic<int> c{0};
        for_each_index_batched_cancelable(10, -1, [&](int, const std::atomic<bool>&){ c.fetch_add(1, std::memory_order_relaxed); });
...
static void test_batched_over_parallelism()
{
    ...
    {
        std::atomic<int> c{0};
        for_each_index_batched(5, 64, [&](int){ c.fetch_add(1, std::memory_order_relaxed); });
        assert_true(c.load() == 5, "over-parallelism batched");
    }
    {
        std::atomic<int> c{0};
        for_each_index_batched_cancelable(5, 64, [&](int, const std::atomic<bool>&){ c.fetch_add(1, std::memory_order_relaxed); });
...
static void test_pool_multiple_exceptions_first_only()
{
    ...
    assert_true(pool.cancel_flag().load(std::memory_order_relaxed), "cancel flag set after exception");
...
static void test_pool_cancel_idempotent_and_post_cancel_submit()
{
    ...
    {
        pool.submit_cancelable([&](const std::atomic<bool>& flag){
            if (flag.load(std::memory_order_relaxed)) return; // should early-return after cancel
    ...
    for (int i = 0; i < 10; ++i)
    {
        pool.submit([&]{ non_cancelable_count.fetch_add(1, std::memory_order_relaxed); });
    }
    pool.wait_idle();
    assert_true(cancelable_count.load() == 0, "post-cancel submit_cancelable should do nothing");
    assert_true(non_cancelable_count.load() == 10, "post-cancel submit should still run");
}
...
static void test_pool_wait_idle_with_long_tasks()
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        pool.submit([&]{ std::this_thread::sleep_for(milliseconds(50)); c.fetch_add(1, std::memory_order_relaxed); });
    }
    for (int i = 0; i < 20; ++i)
    {
        pool.submit([&]{ c.fetch_add(1, std::memory_order_relaxed); });
    ...
    assert_true(dt.count() >= 40, "wait_idle blocks until long tasks complete (>=40ms)");
}
