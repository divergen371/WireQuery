    ...
    for (const addrinfo *ai = res; ai != nullptr; ai = ai->ai_next)
    {
        ...
        if (dedup)
        {
            std::string key = std::to_string(e.af) + '|' + e.ip + '|' +
                              ...
                              std::to_string(e.protocol) + '|' + std::to_string(e.port);
    ...
    bool                      namereqd)
{
    ...
    {
        if (std::string key = std::to_string(af) + '|' + ip; !seen.insert(key).second)
            continue;
        PtrItem item{};
        item.af = af;
        item.ip = ip;
        if (af == AF_INET)
        {
            ...
            if (int rc = getnameinfo(reinterpret_cast<sockaddr *>(&sin), sizeof(sin),
                                     name, sizeof(name), nullptr, 0, flags); rc == 0)
            {
                item.rc   = rc;
        ...
        else if (af == AF_INET6)
        {
            ...
            int flags = NI_NOFQDN | (namereqd ? NI_NAMEREQD : 0);
            if (int rc = getnameinfo(reinterpret_cast<sockaddr *>(&sin6), sizeof(sin6),
                                     name, sizeof(name), nullptr, 0, flags); rc == 0)
            {
                item.rc   = rc;
                item.name = std::string{name};
...
AttemptResult resolve_posix_once(const Options& opt)
{
    ...
    if (opt.reverse)
        result.ptrs = do_reverse_for_entries(result.entries, opt.ni_namereqd);

    result.canon = (res && res->ai_canonname) ? std::string(res->ai_canonname) : std::string();
