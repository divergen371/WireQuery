...
namespace wq
{
        ...
        Cancellation *                                             cancel)
    {
        ...
        auto               set_first_ex = [&](const std::exception_ptr& ep)
        {
            if (!ep) return;
            std::scoped_lock lk(ex_mtx);
            if (!first_ex) first_ex = ep;
        };
        ...
        auto safe_call = [&](int idx)
        {
            ...
            catch (...)
            {
                ...
                else local_cancel.store(true, std::memory_order_relaxed);
            }
        };
    ...
    {
        explicit Impl(int n) : stop(false), active(0), cancel(false)
        {
            if (n <= 0) n = 1;
            workers.reserve(n);
            for (int i = 0; i < n; ++i)
        ...
        void set_first_exception(const std::exception_ptr& ep)
        {
            if (!ep) return;
            std::lock_guard<std::mutex> lk(mtx);
            if (!first_ex) first_ex = ep;
    ...
    void ThreadPool::submit_cancelable(
        std::function<void(const std::atomic<bool> &)> task)
    {
