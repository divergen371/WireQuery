...
std::string build_ndjson_rawdns_init_failed(int t, double ms, const std::string& err, const Options& opt)
{
...
std::string build_ndjson_rawdns_error_with_type(int t, double ms, const std::string& err, const std::string& qtype)
{
    std::ostringstream os;
    os << std::fixed << std::setprecision(3);
    os << "{";
...
std::string build_ndjson_ldns_not_available(int t, double ms, const std::string& err, const Options& opt)
{
    std::ostringstream os;
    os << std::fixed << std::setprecision(3);
    os << "{";
...
std::string build_ndjson_normal(int t, double ms, const std::string& canon,
    ...
    if (!ptrs.empty())
    {
        ...
        for (size_t k = 0; k < ptrs.size(); ++k)
        {
            ...
            os << R"({"family":")" << json_escape(family_str(p.af))
               ...
               << R"(","rc":)" << p.rc;
            if (p.rc == 0)
                os << R"(,"name":")" << json_escape(p.name) << R"(")";
            else
                os << R"(,"error":")" << json_escape(p.error) << R"(")";
            os << "}";
        }
        os << "]";
    }
...
std::string build_ndjson_rawdns_success(int t, double ms, const std::string& qtype,
                                        int rcode,
                                        bool aa, bool tc, bool rd, bool ra, bool ad, bool cd,
                                        size_t answer_count, size_t authority_count, size_t additional_count,
...
std::string build_final_json(const Options& opt,
                             double min_ms, double avg_ms, double max_ms,
                             ...
                             const std::vector<AttemptResult>& attempts)
{
    ...
    os << R"("family":")" << (opt.family == Family::Any ? "any" : (opt.family == Family::IPv4 ? "inet" : "inet6")) << R"(",)";
    os << R"("tries":)" << opt.tries << ",";
    os << R"("service":")" << json_escape(opt.service) << R"(",)";
    os << R"("socktype":")" << json_escape(socktype_str(opt.socktype)) << R"(",)";
    ...
    os << R"("summary":{"min_ms":)" << min_ms << ",\"avg_ms\":" << avg_ms << ",\"max_ms\":" << max_ms << ",\"count\":" << attempts.size() << "},";
    if (!pctl_values.empty())
    {
        ...
        {
            if (i) os << ",";
            os << R"("p)" << pctl_values[i].first << R"(":)" << pctl_values[i].second;
        }
        os << "},";
    }
    os << R"("attempts":[)";
    for (size_t i = 0; i < attempts.size(); ++i)
    {
        ...
        if (i) os << ",";
        os << "{";
        os << R"("try":)" << (i + 1) << R"(,"ms":)" << ar.ms << R"(,"rc":)" << ar.rc;
        if (!ar.error.empty()) os << R"(,"error":")" << json_escape(ar.error) << R"(")";
        if (!ar.canon.empty()) os << R"(,"canon":")" << json_escape(ar.canon) << R"(")";
        os << R"(,"addresses":[)";
